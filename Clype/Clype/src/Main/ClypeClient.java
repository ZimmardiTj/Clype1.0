...
public class ClypeClient  {
	...
	Data.ClypeData dataToSendToServer;
	Data.ClypeData dataToRecieveFromServer;
	Scanner inFromStd;
...
//		}
	...
	public ClypeClient(String userName, String hostName){
		this(userName,hostName,DEFAULT_PORT);//Calling other constructor, passing default port
		if(userName.equals(null)) {
			throw new IllegalArgumentException("Username cannot be empty");
		}
		if(hostName.equals(null)) {
	...
	public ClypeClient(String userName){
		this(userName,"localhost");//Default hostname and port number
		if(userName.equals(null)) {
			throw new IllegalArgumentException("Username cannot be empty");
		}
	...
	public void start(){
		...
		System.out.println(
				...
				"DONE : Log Out\n"+
				"SENDFILE Filename : Send a file with name given\n"+
				"LISTUSERS : List all users in session");
